...
-- Project bookmark
create table
  public.project_bookmark (
    project_bookmark_id bigint generated by default as identity,
    _created_at timestamp with time zone not null default now(),
    project_repo_id bigint not null,
    user_id uuid not null,
    constraint project_bookmark_pkey primary key (project_bookmark_id),
    constraint project_bookmark_project_repo_id foreign key (project_repo_id) references project_repo (project_repo_id) on delete cascade,
    constraint project_bookmark_user_id_fkey foreign key (user_id) references auth.users (id) on delete cascade
  ) tablespace pg_default;

-- Project Category to Project Repo
create table
  public.project_category_and_project_bookmark (
    project_category_id bigint not null,
    project_bookmark_id bigint not null,
    constraint project_category_and_project_bookmark_pkey primary key (project_category_id, project_bookmark_id),
    constraint project_category_and_project_bookmark_project_category_id_fkey foreign key (project_category_id) references project_category (project_category_id) on delete cascade,
    constraint project_category_and_project_bookmark_project_bookmark_id_fkey foreign key (project_bookmark_id) references project_bookmark (project_bookmark_id) on delete cascade
  ) tablespace pg_default;
