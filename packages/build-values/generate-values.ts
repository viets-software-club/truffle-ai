...
const outConfigMap = path.resolve(
	path.join(mainModuleDir, '../../outputs/_configMaps')
)
const outSecret = path.resolve(
	path.join(mainModuleDir, '../../outputs/_secrets')
)
console.info(`Writing ConfigMaps to ${outConfigMap}`)
await generateNamespacedResource('ConfigMap', '../../_configMaps', outConfigMap)
console.info('-'.repeat(20))
console.info(`Writing Secrets to ${outSecret}`)
await generateNamespacedResource('Secret', '../../_secrets', outSecret)
console.info('-'.repeat(20))
