...
const CategoryModal: FC<CategoryModalProps> = ({
	...
	const [{ fetching }, renameBookmarkCategoryMutation] =
		useRenameBookmarkCategoryMutation()

	// Sends mutation that renames the bookmark category
	const renameBookmarkCategory = async () => {
		try {
			const res = await renameBookmarkCategoryMutation({
				oldCategory: category,
				newCategory
			})
			const responseCode = parseInt(
				res?.data?.renameBookmarkCategory?.code as string,
				10
			)
			...
			if (responseCode >= 200 && responseCode < 300) {
				...
				setError(
					res?.data?.renameBookmarkCategory?.message || defaultErrorMessage
				)
			}
		} catch (e) {
	...
	return (
		<Modal isOpen={open} onClose={close}>
			...
			>
				...
				{/* Error message */}
				{error && (
					<p className="text-sm text-red-400">{error ?? defaultErrorMessage}</p>
				)}
