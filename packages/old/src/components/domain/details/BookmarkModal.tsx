...
const BookmarkModal: FC<BookmarkModalProps> = ({
	...
	const [{ fetching: fetchingAdd }, addBookmarkMutation] =
		useAddBookmarkMutation()
	const [{ fetching: fetchingDelete }, deleteBookmarkMutation] =
		useDeleteBookmarkMutation()
		...
		try {
			...
			if (isBookmarked) {
				...
				const responseCode = parseInt(
					res?.data?.editBookmarkCategory?.code as string,
					10
				)
				...
				if (responseCode >= 200 && responseCode < 300) {
					...
					setErrors({
						default:
							res?.data?.editBookmarkCategory?.message || defaultErrorMessage
					})
				}
			} else {
				...
				const responseCode = parseInt(
					res?.data?.addBookmark?.code as string,
					10
				)
				...
				if (responseCode >= 200 && responseCode < 300) {
					...
					// Otherwise, show error message
					setErrors({
						default: res?.data?.addBookmark?.message || defaultErrorMessage
					})
				}
			}
		} catch (e) {
	...
	const deleteBookmark = async () => {
		try {
			...
			const responseCode = parseInt(
				res?.data?.deleteBookmark?.code as string,
				10
			)
			...
			if (responseCode >= 200 && responseCode < 300) {
				...
				// Otherwise, show error message
				setErrors({
					default: res?.data?.deleteBookmark?.message || defaultErrorMessage
				})
			}
		} catch (e) {
			setErrors({ default: defaultErrorMessage })
	...
	return (
		<Modal isOpen={open} onClose={close}>
			...
			>
				...
				{/* Error message */}
				{errors.default && (
					<p className="text-sm text-red-400">{errors.default}</p>
				)}
