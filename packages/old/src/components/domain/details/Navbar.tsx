...
import { Project, useProjectIdsQuery } from '@/graphql/generated/gql'
import {
...
const Navbar = ({ loading }: NavbarProps) => {
	...
	const { filters: trendingFilters, sorting: trendingSorting } =
		useTrendingProjectsState()
	const { filters: bookmarkFilters, sorting: bookmarkSorting } =
		useBookmarkedProjectsState()
	const { filters: categoryFilters, sorting: categorySorting } =
		useCategoryProjectsState()
	...
	const projects = projectIds?.projectCollection?.edges?.map(
		(edge) => edge.node
	) as Project[]
