const transformData = (
	data: SidebarProjCategoriesToProjBookmarksQuery | undefined
): React.ComponentProps<typeof BookmarkGroup>[] | null => {
	if (!data?.projCatCollection?.edges) return null
	return data.projCatCollection.edges.map(({ node }) => {
		const items = node.projCatAndProjBookmarkCollection.edges.map(
			({ node }) => {
				const repoName = node.projBookmark.projRepo.gthbRepo.gthbRepoName
				const ownerLogin =
					node.projBookmark.projRepo.gthbRepo.gthbOwner.gthbOwnerLogin
				return {
