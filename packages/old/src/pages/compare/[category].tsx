...
import {
	...
	defaultSort
} from '@/components/domain/projects/types'
...
const ComparePage: NextPageWithLayout = () => {
	...
	const [
		{ data: bookmarkData, fetching: bookmarksFetching, error: errorBookmarks }
	] = useBookmarkIdsQuery({
	...
	useEffect(() => {
		if (urqlData) {
			...
			const projectData = urqlData?.projectCollection?.edges?.map(
				(edge) => edge.node
			) as Project[]
			setData(projectData)
			setLoading(false)
	...
	useEffect(() => {
		if (lastViewedPage === category && !bookmarksFetching) {
			setCategoryFilters({
				...filters,
				id: bookmarkIds ? { in: bookmarkIds } : undefined
			})
	...
	return (
		<ProjectsTable
			...
			beforeTable={
				<CompareContent data={data} category={category} loading={loading} />
			}
			hideTimeFrame
			showPinned
			rowPinning={{
			...
			setRowPinning={(
				pinning:
					| RowPinningState
					| ((prevState: RowPinningState) => RowPinningState)
			) => {
				setRowPinning({
					...rowPinning,
					[category]: [
