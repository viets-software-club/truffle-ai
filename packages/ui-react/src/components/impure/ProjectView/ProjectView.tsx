...
import Table from '@/components/pure/Table/Table'
import TopBar from '@/components/domain/projects/TopBar'
import createColumns, {
	PercentileStats
} from '@/components/domain/projects/columns'
...
import ProjectListItem from '../../domain/projects/ProjectListItem'
...
const ProjectsTable: FC<ProjectsTableProps> = ({
	...
	const { columnVisibility, setColumnVisibility } =
		useProjectTableVisibilityState()

	const columns = useMemo(
		() => createColumns(percentileStats, showPinned),
		[percentileStats]
	)
				...
				{!error && (
					<div className="mx-4 my-2 md:w-full md:max-w-[calc(100vw-32px)] md:overflow-hidden lg:mx-6 lg:my-3.5 lg:max-w-[calc(100vw-224px-48px)]">
						...
						{fetching ? (
							...
							<p className="w-full p-12 text-center text-sm text-white/75">
								No projects found
							</p>
						) : (
