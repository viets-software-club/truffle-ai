    ...
    <MenuItemsTransition>
      <Menu.Items className='absolute left-0 z-30 mt-2 origin-top-right rounded-md border border-white/5 bg-white/5 p-1 backdrop-blur-xl focus:outline-none'>
        {props.items.map(item => (
            <Menu.Item key={item.id} as='button' onClick={item.onClick} className='min-w-[150px] rounded-md p-2 text-left text-sm text-white/90 transition-colors duration-75 hover:bg-white/5'>
              {item.title}
            </Menu.Item>
          )
        )}
