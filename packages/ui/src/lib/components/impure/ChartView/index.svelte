...
const getChartData = () => {
	if (data) {
		let datasets = data.map((repo: any) => {
			const chartData = repo.xyData; // e.g. forks, issues, stars array
		...
		if (isNormalized) {
			const earliestDate = Math.min(
				...datasets.flatMap((dataset: any) => {
					return dataset.data.map((point: { x: DateTime; y: number }) =>
						point.x.toMillis(),
					);
				}),
			);
			datasets = datasets.map((dataset: any) => ({
				...
				data: dataset.data.map((point: { x: DateTime; y: number }) => ({
					...
					x: DateTime.fromMillis(
						point.x.toMillis() -
							(Math.min(...dataset.data.map((p: any) => p.x.toMillis())) -
								earliestDate),
					),
				})),
			}));
...
function parseAndAccumulateDate(data: { edges: any }, field: string) {
	return [...data.edges]
		...
		.map((forkEdge, index, array) => {
			if (
				index !== 0 &&
				array[index - 1].node.amount > Number.parseInt(forkEdge.node.amount)
			) {
				return {
					y: Number.parseInt(array[index - 1].node.amount) as number,
					x: DateTime.fromISO(forkEdge.node[field]),
				};
			}

			return {
				y: Number.parseInt(forkEdge.node.amount) as number,
				x: DateTime.fromISO(forkEdge.node[field]),
			};
		});
				...
				.then((res) => {
					data = res.data.gthbRepoCollection?.edges.map(({ node }, i) => {
						return {
							...
							xyData: parseAndAccumulateDate(
								node.gthbStarHistCollection,
								"gthbStarHistDate",
							),
							color: colors[i],
						};
					...
					variables: {
						...
						issueFilter:
							selectedRange.value === RANGE_ALL_YEAR
								...
								: {
										gthbIssueHistDate: {
											gte: calcPrevDateByRange().toString(),
										},
									},
						// issueOrderBy:
						// 	selectedRange.value === RANGE_ALL_YEAR
...
<div class="ml-auto md:ml-0 flex gap-4 items-center mb-2 w-full border-b p-4">
	...
	{#if isNormalizable}<Button on:click={handleNormalize}
