<script lang="ts">
	...
	const columns = table.createColumns([
		...
		table.column({
			...
			header: 'Stars',
			cell: ({ value }) =>
				createRender(TextIcon, { icon1: StarIcon, value: value, max: data.maxStars }).slot(value)
		}),
		table.column({
			accessor: 'issues',
			header: 'Issues',
			cell: ({ value }) =>
				createRender(TextIcon, { icon1: CircleDotIcon, value: value, max: data.maxIssues }).slot(
					value
				)
		}),
		table.column({
		...
		table.column({
			...
			header: 'Contrib.',
			cell: ({ value }) =>
				createRender(TextIcon, { icon1: UsersIcon, value: value, max: data.maxContributors }).slot(
					value
				)
		}),
		table.column({
