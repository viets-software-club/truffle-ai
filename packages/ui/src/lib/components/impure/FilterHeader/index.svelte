...
type Filter = { id: ID; operator: string; value: string | number };
type Props = {
	options?: {
	...
	data: {
		...
		onFilterOpenChange: (id: ID, filter: Filter, isOpen: boolean) => void;
		sortableItems: {
			id: ID;
			title: string;
			icon1: any;
...
const { options, left, data = $bindable() }: Props = $props();
let {
	time,
	columns,
	filterableItems,
...
const getFilterOnSelectedChangeFunc =
	(id: ID, filter: Filter) => (obj: any) => {
		onFilterChange(id, {
			...filter,
			operator: (obj as { value: string }).value,
		});
	};
const getFilterOnOpenChangeFunc =
	(id: ID, filter: Filter) => (isOpen: boolean) => {
		onFilterOpenChange(id, filter, isOpen);
	};
                    ...
                    <SortableList class="" onUpdate={handleSortableListUpdate}>
                        {#each sortableItems as { id, title, icon1: Icon1, icon2: Icon2, disabled, isAsc }, i}
                            {#key id}
                                {#key disabled}
                                    <div
                                        onclick={getHandleSortItemClickFunc(id)}
                                        role="menuitem"
                                        tabindex={null}
                                        onkeydown={null}
                                    >
            ...
            <DropdownMenu.Root>
                ...
                <DropdownMenu.Content>
                    {#each filterableItems as { id, title, icon1: Icon1, icon2: Icon2 }, i}
                        <div onclick={getFilterAddClickHandler(id)} onkeydown={null} role="button" tabindex={null}>
            ...
            {#each filterableItems as filterableItem}
                {#if filterableItem.filters}
                    {#each filterableItem.filters as filter}
                        <DropdownMenu.Root onOpenChange={getFilterOnOpenChangeFunc(filterableItem.id, filter)}>
                            <DropdownMenu.Trigger asChild let:builder>
                                <Button variant="outline" builders={[builder]} size="sm" class="mr-2 text-[0.8rem]">
                                    {filterableItem.title}
                                    {#key filter.operator}
                                        {#if filterableItem.type === "number"}
                            ...
                            <DropdownMenu.Content class="p-6">
                                ...
                                >
                                    ...
                                    <Select.Content class="w-[4rem]">
                                        ...
                                        <Select.Item value="ilike">ilike (%)</Select.Item>
                                        {/if}
                                    </Select.Content>
                                </Select.Root>
                                <Input
                                    placeholder={filterableItem.type === "number" ? "0" : ""}
                                    type={filterableItem.type}
                                    class="w-[9rem]"
                                    value={filter.value}
