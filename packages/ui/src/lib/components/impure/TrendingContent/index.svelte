<script lang="ts">
	...
	const createGraphQLVariables = () => {
		return {
			dateRange: getDateRange(filterData.time),
			filter: {
	...
	const loadStorage = () => {
		if(localStorage.getItem('trendingFilter')) {
			const trendingLocalStore = localStorage.getItem('trendingFilter');
	...
	const updateStorage = async () => {
		return new Promise((resolve) => {
			localStorage.setItem('trendingFilter', JSON.stringify({
				columns: filterData.columns.map((col) => ({ id: col.id, checked: col.checked })),
